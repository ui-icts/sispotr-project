...
<div class="modal-body">
         <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','4');" checked="checked"/>Start Position</label>
          <br/>
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','5');" checked="checked"/>POTS</label>
          <br/> 
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','6');" checked="checked"/>Percentile (%&nbsp;worse)</label>
          <br/>
          
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','7');" checked="checked"/>Guide (Antisense)</label>
          <br/>
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','8');" checked="checked"/>Passenger (Sense)</label>
          <br/>
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','9');" checked="checked"/>Seed Sequence</label>
          <br/> 
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','10');" checked="checked"/>miRNA seed</label>
          <br/>
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','11');" checked="checked"/>miRNA conservation</label>
          <br/>
          <label><input type="checkbox"  onclick="toggleCol('#table_${aid}','12');" checked="checked"/>% G/C</label>
          </div>
...
<table class="display" id="table_${aid}">
			<thead align="center">
				<tr>
		  <th></th>
		  <th>Select</th>
		  ...
		  ${siteContentMap['resultstable.header']}
            ...
            <jsp:include page="outputHeader.jsp"></jsp:include>
...
<c:if test="${ pots <= 50}">
<i class="icon-ok-circle icon-green" title="POTS Score &lt;= 50"></i>
</c:if>
</td>			
<td>
<button id="add${aid}${row[0]}"  type="button" title="add" onclick="addToCart('${aid}','${status1.index}');$('#add${aid}${row[0]}').attr('disabled','disabled');">add</button>    </td>
<td>
    <div class="btn-group">
...
$(".display").dataTable( {
        ...
        "aaSorting": [[ 3, "asc" ]],                                // sor the first col asc on initialization
        "sPaginationType": "full_numbers",                          // pagination style
		"aoColumns" : [{ "bSortable":false },{ "bSortable":false }, null,null,null,null,null,null,null,null,null,null,null,null],
		 "oLanguage": {
...
// 	$("#recomputeButton").attr("disabled",false);
// 	$("#statusMessage").html("Update successful");
// 	setTimeout(function() {
